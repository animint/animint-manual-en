---
title: The `clickSelects` keyword
---
...
![Sketch of World Bank data visualization with `clickSelects`](ch04-legend-clickSelects.png)
...
![Sketch of World Bank data visualization with text for one selected country](ch04-viz-text-single.png)
...
on the scatterplot. We can do that by adding a `geom_text()` with
...
![Sketch of World Bank data visualization with text for several selected countries](ch04-viz-text.png)
...
![Sketch of World Bank data visualization with time series](ch04-ggplot-time-series-plot.png)
...
**Animation exercise:** make the data viz animated by specifying the
  `time` option,
  [as explained in Chapter 3](/ch03#animation-time).
...
state is displayed](/ch06#display-selection-state).
...
* When `alpha=0.75` is used with `geom_point()` on the usual R graphics device, how much opacity/transparency is
...
Next, [Chapter 5](/ch05) explains several different
