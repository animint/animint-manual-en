...
It should be set to an identifier with common data across different `showSelected` subsets.
...
To study compression, we force creating separate files for this geom, by adding the `chunk_vars` parameter:
...
The output above is a list of four data tables which exhibit compression:
...
In conclusion, we have seen several examples about how `aes(group)` can be used for data compression.
