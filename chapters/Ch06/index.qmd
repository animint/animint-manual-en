...
The output above shows a table with one row per year and country.
...
animint_tsv <- function(...){
  ...
  sapply(tsv.files, fread, nrow=3, simplify=FALSE)
}
```
...
To study compression, we force creating separate files for this geom, by adding the [`chunk_vars` parameter](#chunk-vars):
...
They have the same columns as the previous TSV file (output with no `chunk_vars`), so there is no compression.
...
To get compression when `group=month`, we would need at least two variables that are specific to each group across years, as shown in the example below:
...
* The last one is the common chunk, with one row per country/month, and columns `group`, `colour`, `fill`, `x`. These are the columns which have common values in each group (same for each month, across years).

Below, we use paths instead of points.
...
which we can do using the code below.

```{r}
grid.list <- lapply(country.dt[, .(country,year,month)], unique)
grid.dt <- do.call(CJ, grid.list)
na.dt <- country.dt[grid.dt, on=names(grid.dt)]
animint_tsv(
  ggplot()+
    geom_path(aes(
      month, value, color=country, group=country),
...
The output above shows a common chunk data table with columns for `group`, `colour`, `showSelected2`, and `x`. Whereas `x` has different values in each row, `colour` and `showSelected2` do not (so some efficiency is lost in this repetition). Also note above that the `chunk1.tsv` file has columns `row_in_group` and `na_group` which are used to join these data with the common chunk file. These columns are not necessary in the other two chunks, because there are no missing values at the beginning of the data sequence.
