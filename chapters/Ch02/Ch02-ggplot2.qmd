...
![World Bank scatterplot](../Ch02/Ch02-viz-scatter.png)
...
![multi-layer WorldBank data viz](../Ch02/Ch02-two-layer-viz.png)
...
**Exercise**: try changing the `region` legend to an `income`
  ...
  details](../Ch99/Ch99-appendix.html#manual-color-legends).
...
![WorldBank data viz with two plots](../Ch02/Ch02-viz-two-plots.png)
...
![World Bank aligned plot](../Ch02/Ch02-viz-aligned.png)
...
* increasing plot width using `theme_animint()`, see [Chapter 6](../Ch06/Ch06-other.html#plot-height-width) for more info.
...
![World Bank panels plot](../Ch02/Ch02-viz-panels.png)
...
for(show.year in c(1975, 1985, 1995)){
  ...
  show.path.list[[paste(show.year)]] <- data.frame(
    show.year, subset(
      WorldBank, show.year - 5 <= year & year <= show.year))
}
