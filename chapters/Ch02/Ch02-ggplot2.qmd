...
([Wilkinson, 2005](http://www.springer.com/us/book/9780387245447?wt_mc=GoogleBooks.GoogleBooks.3.EN&token=gbgen#otherversion=9780387286952)).
Wickham later implemented several ideas from the grammar of graphics in the
`ggplot2` R package
([Wickham, 2009](https://www.amazon.com/dp/0387981403/ref=cm_sw_su_dp?tag=ggplot2-20)).
...
[Chapter 16](../Ch16/Ch16-change-point.html), which uses the `penaltyLearning`
...
We use a data set from the World Bank as an example.
...
![Sketch of scatter plot based on World Bank data](../Ch02/Ch02-viz-scatter.png)
...
![Sketch of multi-layer visualization using World Bank data](../Ch02/Ch02-two-layer-viz.png)
...
You may want to use `scale_color_manual` with a sequential color palette, see `RColorBrewer::display.brewer.all(type="seq")` and [read the appendix for more details](../Ch99/Ch99-appendix.html#manual-color-legends).
...
**Exercise:** Re-make this data visualization using `aes(tooltip)`, which is a new feature in `animint2` (not present in `ggplot2`), and is discussed in [Chapter 5](../Ch06/Ch06-other.html#new-aesthetics).
...
![Sketch of visualization with two plots based on World Bank data](../Ch02/Ch02-viz-two-plots.png)
...
![Sketch of multi-panel visualization of World Bank data using different geoms in each panel (aligned axes)](../Ch02/Ch02-viz-aligned.png)
...
We call this the
[Space saving facets idiom](../Ch99/Ch99-appendix.html#space-saving-facets), 
...
![Sketch of multi-panel visualization with same geoms in each panel, to compare World Bank data in different years](../Ch02/Ch02-viz-panels.png)
...
[list of data tables idiom](../Ch99/Ch99-appendix.html#list-of-data-tables),
...
subsets over time. In the [next chapter](../Ch03/Ch03-showSelected.html), we
...
Next, [Chapter 3](../Ch03/Ch03-showSelected.html) explains the `showSelected`
