...
ggplot()+
  ...
  geom_segment(aes(
    ...
    data=score_stats)+
  scale_x_continuous(
    "AUC (mean ± SD over 3 folds in CV)",
...
score_stats[, hjust := {
  M <- max(hi)
  m <- min(lo)
  f <- function(a)m+a*(M-m)
  fcase(
    classif.auc_mean<f(0.3), 0,
    classif.auc_mean>f(0.7), 1,
    default=0.5)
}, by=task_id]
gg.rank.text <- gg.rank+
  geom_text(aes(
    classif.auc_mean, auc.task.rank+0.1,
    hjust=hjust,
...
(pval_dt <- data.table(comparison_i=1:3)[, {
  ...
  i_wide <- dcast(i_long, iteration ~ rank, value.var="classif.auc")
...
* In visualizations with ROC curves, add `aes(color)` or `aes(fill)` to the other plots, with a consistent color scheme that matches the ROC curves.
* Instead of showing Best TPR for selected FPR, make a re-design that shows best FPR for selected TPR.
