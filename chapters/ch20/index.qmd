...
The figure above shows AUC for every data set, learning algorithm, and train/test split in cross-validation.
...
(gg.auc <- ggplot()+
   theme_bw()+
   ##facet_wrap( ~ task_id, scales="free", ncol=1)+
   facet_grid(. ~ task_id, scales="free")+
   scale_x_continuous(
...
animint(
  testAUC=gg.auc+
    ggtitle("Click to select data set and cross-validation split")+
    theme_animint(width=1000, height=200, last_in_row=TRUE),
  roc=gg.roc.text+
    ggtitle("ROC curves for selection")+
    theme_animint(width=400, height=400)+
    geom_label_aligned(aes(
      Inf, algo.rank*0.1, color=learner_id,
...
We additionally keep only a max of 20 FPR values, almost evenly spaced across the range of FPR values.
...
The output above shows ROC curves in each panel, with vertical lines showing the FPR thresholds which can be selected by the user.
...
(gg.tpr <- ggplot()+
   ...
   geom_point(aes(
     learner_id, best_TPR, color=learner_id),
     data=best_TPR_show,
     size=5,
     showSelected=c("task_it","FPR"))+
   geom_text(aes(
     learner_id, best_TPR, label=sprintf("%.4f", best_TPR)),
     data=best_TPR_show,
     showSelected=c("task_it","FPR")))
```
...
(viz.select.fpr <- animint(
  testAUC=gg.auc+
    ggtitle("Click to select data set and cross-validation split")+
    theme_animint(width=1000, height=200, colspan=2, last_in_row=TRUE),
  roc=gg.roc.fpr+
    ggtitle("ROC curves for selection")+
    theme_animint(width=400, height=400),
  TPR=gg.tpr+
...
(pval_dt <- data.table(comparison_i=1:3)[, {
  ...
  paired <- with(i_wide, t.test(
    lo, hi, alternative = "l", paired=TRUE))
  unpaired <- with(i_wide, t.test(
    lo, hi, alternative = "l", paired=FALSE))
  data.table(
...
ggplot()+
  ...
  geom_point(aes(
    ...
    data=details_dt)+
  scale_y_continuous("Validation AUC")+
  facet_grid(task_id ~ iteration, labeller=label_both, scales="free")
```
...
* Add an error rate plot to the display with interactive ROC curves where you can select FPR. Hint: you will need to join the number of labels to the ROC curve table.
* In the "Best TPR for selected FPR" plot, add `geom_text()` which shows the selected value of FPR.
