...
(pval_dt <- join_dt[, {
  ...
  unpaired <- t.test(
    lo.auc, hi.auc, alternative = "l", paired=FALSE)
  data.table(
...
* In visualizations with ROC curves, add `aes(color)` or `aes(fill)` to the other plots, with a consistent color scheme that matches the ROC curves. Make sure there is only one legend, and that it can make all data for a learner disappear, across all plots.
