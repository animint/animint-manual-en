...
So we would like to run the learning algorithms which are implemented in these different R packages, and we would like to standardize the results so they can be analyzed together to determine which algorithm is most accurate.
...
And how can we convert the predictions from each package into a standard format that allows easy comparison of results?
...
Some of the code in this chapter is adapted from a tutorial about related static data visualizations [@Hocking2025-mlr3-cluster].
...
* `spam` represents email classification into spam or not, based on a bag of words vector [@spambase].
* `sonar` represents a problem of discriminating metal and rock, based on a vector of sonar frequency bands [@gorman1988analysis].
* `vowel` represents a problem of discriminating vowel sounds, using ten features extracted from audio data [@vowel].
* `waveform` is a simulated binary problem [@Hastie2009].
* `zip` represents handwritten digit recognition, where features are a vector representing 16x16 pixel intensities [@lecun1990].
...
* `featureless` is the standard baseline which ignores all of the input features, and always predicts the most frequent class that was seen in the training data.
...
Because our goal in this chapter is to explain data visualization techniques, we have pre-computed the results, which are saved in `library(animint2data)`.
The code block below downloads these pre-computed results.
...
If you want to re-run the benchmark, you could run the line of code above with `benchmark()`.
...
To do the computation even faster, a compute cluster can be used for parallelization [@Hocking2025-mlr3-cluster].
...
* `classif.auc` is the area under the ROC curve.
* `classif.ce` is the classification error rate, which is the number of incorrectly predicted labels (positive or negative), divided by the total number of labels.
* `classif.tpr` is the true positive rate, which is the number of correctly predicted positive labels, divided by the total number of positive labels.
* `classif.fpr` is the false positive rate, which is the number of incorrectly predicted negative labels, divided by the total number of negative labels.
...
The output above has one row per combination of task/learner/split, and two columns for evaluation metrics: ROC-AUC and classification error.
...
We see in the figure above that `featureless` is no longer present.
...
* in the top left plot, you can click test AUC values to select a data set and cross-validation iteration.
* in the top right plot, you can click a vertical line to select a FPR threshold.
* in the bottom plot, you can see the best TPR values for the selected FPR threshold.
...
After we divide the data into train and test sets, we then divide the train set into subtrain and validation sets.
We would like to select the number of neighbors that maximizes the validation AUC, and then use this number of neighbors to compute predictions on the test set.
For each test set, there could be a different selected number of neighbors.
To visualize the number of selected neighbors, we compute a table with one row per train/test split, and columns which contain information on how many neighbors were used for prediction.
...
* color to highlight the best numbers of neighbors according to validation AUC values, and
* vertical lines to highlight the selected number of neighbors.

We can see how the AUC was selected for each task and iteration.

* Three data sets (`sonar`, `spam`, `vowel`) have a clear max at an intermediate number of neighbors, which avoids both over- and under-fitting. For `vowel` iteration 2, max AUC occurs at the extreme value of 1 neighbor, which is the smallest hyper-parameter in our search. Ordinarily this would suggest to enlarging the search space, in hopes that an even smaller hyper-parameter may give even better validation AUC. However, there is no smaller value of neighbors than 1, so there is no possibility of underfitting with the number of neighbors hyper-parameter.
* Two data sets (`waveform`, `zip`) have max AUC at the largest number of neighbors (40), which suggests overfitting. These data suggest that an even larger AUC may be obtained by increasing the number of neighbors past 40, until we see a clear decrease of the AUC values.
...
Above we can see that the linear model can overfit if not properly regularized in some data sets (`sonar`, `vowel`, `waveform`).
In the other data sets (`spam` and `zip`), we see a curve that always decreases, which suggests that the linear model underfits, and that better predictions may be possible using non-linear learners.
Synthesizing the results from previous sections, we can check if that is true.

* In `spam`, the `torch_dense_50` algorithm is more accurate than `cv_glmnet`, which indicates that the linear model is not powerful enough for optimale prediction in these data.
* In `zip`, all algorithmes yield perfect predictions, which indicates that these data represent a problem which is very easy for all learners (even the linear model).
