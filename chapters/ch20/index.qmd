...
Note that we must set `predict_type="prob"` so that the learned model outputs real-valued scores (necessary for computing ROC-AUC) instead of only predicted class labels.
...
The output above has one row per combination of task/learner/split.
...
```{r}
score_dt[, task_iteration := paste(task_id, iteration)]
roc_dt <- score_dt[, roc := {
