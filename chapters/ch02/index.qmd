...
Wickham later implemented several ideas from the grammar of graphics in the `ggplot2` R package [@wickham2009ggplot2].
...
Since many concepts from static graphics are also useful in interactive graphics, `animint2` is implemented as a fork of `ggplot2`, with several modifications which make it easy to create interactive data visualizations.
The advanced interactive features in `animint2` are only possible because the `ggplot2` authors freely shared their code (thanks very much!).
...
Starting in 2013, Toby Hocking and students created the `animint` package, which depends on the `ggplot2` package.
...
In fact, it is recommended to use `library(animint2)` without attaching `ggplot2`.
However it is fine to use `animint2` along with packages that import/load `ggplot2`.
For an example, see [Chapter 16](/ch16), which uses the `penaltyLearning` package (which imports `ggplot2`).
...
In this book, we use the word "sketch" to mean "A rough design, plan, or draft," [@wiktionary-sketch].
...
Internally, the `animint()` function creates a list of class `"animint"`, and then R runs the `print.animint()` function via the S3 object system [@S3_object_system].
...
If, when you run the code above, the animint does not render in your web browser for some reason (for example if you see a blank web page), then please consult our wiki FAQ [@wiki_FAQ] which will help you find a solution.
