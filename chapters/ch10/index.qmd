...
Our goal in this chapter is to create an interactive version of Figure 13.4 from
[Elements of Statistical Learning](http://statweb.stanford.edu/~tibs/ElemStatLearn/) [@Hastie2009].
That Figure consists of two plots, which we sketch below using the interactive `clickSelects` and `showSelected` keywords.

![Sketch of data visualization of K-Nearest Neighbors classification](ch10-viz-interactive.png)

Left: mis-classification error curves, as a function of the number of neighbors.
  
  * `geom_tallrect()` is for selecting `neighbors`.
...
Right: data and decision boundaries in the two-dimensional input feature space, for the currently selected value of `neighbors`.
...
We begin by creating a static version of this visualization, for one choice of the number of `neighbors`.
...
We begin with a re-design of the error curves plot, with the following changes:

* add a selector for the number of neighbors, using `geom_tallrect()`.
...
* change the error bars with `geom_linerange()` to error bands with `geom_ribbon()`.
