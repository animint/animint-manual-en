...
* `round_*` columns have been rounded to the nearest integer, and are used to define the heat map tile `x` and `y` positions.
* `rel_*` columns will be used for `x` and `y` positions in the zoomed display, and are units relative to the corresponding `round_*` values.
