...
Starting with [chapter 2](ch02), we will show how plot sketches can be translated into R ggplot code.
Many readers may already be familiar with the `ggplot2` package, which implements static ggplots, in which there are no interactions possible [@wickham2009ggplot2].
The interactive ggplots of `animint2` are only possible thanks to the generosity of the `ggplot2` authors, whose freely available code was forked to create `animint2`.
Readers already familiar with `ggplot2` will be able to quickly learn the advanced interactive features of `animint2`, in which direct or indirect manipulation can be used to change the displayed subset of data throughout several linked ggplots.
...
It explains how `animint2` ggplots can be rendered on web pages, with interactive legends for categorical variables.
...
Chapter 3 then explains how the `showSelected` keyword makes it possible for a geoms such as points or lines to change their subsets of displayed data, based on the interactively-determined value of a selection variable.
...
The `clickSelects` keyword makes it possible for the user to change a selection variable by direct manipulation (clicking on a plotted data element such as a point or line).

[Chapter 5](ch05) explains several different ways to
share your interactive data visualizations on the web.

[Chapter 6](ch06) covers some other features of `animint2`,
including how to specify hyperlinks, tooltips, data-driven selector
variable names.

[Chapter 7](ch07) covers the limitations of the
...
chapter explains how to make data visualizations for a particular data set
or statistical algorithm.

[Chapter 8](ch08) explains how to create a multi-panel interactive World Bank
data visualization.

[Chapter 9](ch09) shows a visualization of data from cyclists in Montreal.

[Chapter 10](ch10) explains how to create an
interactive re-design of the nearest neighbors data visualization from
the Elements of Statistical Learning book [@Hastie2009].

[Chapter 11](ch11) shows a data visualization that explains the Lasso, a
machine learning model for regularized regression.

[Chapter 12](ch12) shows a data visualization that explains support vector
machines (SVM), a machine learning model for binary classification.

[Chapter 13](ch13) explains how to create an
interactive visualization that explains the Poisson regression model.

[Chapter 14](ch14) shows an example of how to create
data-driven selectors using named `clickSelects` and `showSelected` in an
interactive visualization of a peak detection model.

[Chapter 15](ch15) explains how to create an interactive
visualization of the Newton root-finding algorithm.

[Chapter 16](ch16) explains how to create an interactive
visualization of an optimal changepoint detection model.

[Chapter 17](ch17) explains how to create an
interactive visualization of the k-means clustering algorithm.

[Chapter 18](ch18) explains how to create an
...
[Chapter 20](ch20) explains how to visualize results of machine learning benchmarks, including plots of ROC curves, test AUC, and validation set metrics used for model selection.
...
different types of information. The word "data" comes from Latin,
its plural form "datum," refers to one observation/row of a data
...
* Does this new lung cancer treatment work better than the old
  ...
  tumoral marker measurement, etc.
...
the famous tea tasting experiment proposed by Ronald Fisher [@the_famous_tea_tasting_experiment_proposed_by_Ronald_Fisher]. A
...
Keeling Curve [@Keeling_Curve]. It is
...
the combustion of fossil fuel" [@keeling1960concentration].
