...
For example I executed the World Bank data viz example code to create the `WorldBank-facets` directory on my personal computer.[^1]
I copied that directory to my lab web server using `rsync -r WorldBank-facets/ monsoon.hpc.nau.edu:genomic-ml/WorldBank-facets/` and so I can view it on my university web server.[^2]
...
* Commit and push the R code file to GitHub, so you can get a public URL that you can add in your R code as the [`source` option](https://animint-manual-en.netlify.app/ch06/ch06-other#source-option), for example `source="https://github.com/tdhock/cs499-spring2020/blob/master/2020-02-03-capacity/figure-quadratic-interactive.R"`.
* Add a [`title` option](https://animint-manual-en.netlify.app/ch06/ch06-other#title-option) in your R code, for example `title="Overfitting using linear model polynomial degree"`.
* Use [HTML table layout](https://animint-manual-en.netlify.app/ch06/ch06-other#html-table-layout) to make sure that the ggplots in your visualisation always have the same arrangement (in any size web browser window).
...
Next, [Chapter 6](../Ch06/Ch06-other.html) explains the different options that
