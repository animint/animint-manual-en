...
most
[animated data visualizations](../Ch03/Ch03-showSelected.html#animation-time),
...
random.counts <- data.table(
  ...
  stack = rep(rep(factor(c("lower","upper"), c("upper","lower")), each = 5), 2),
...
[selector.types](../Ch04/Ch04-clickSelects.html#selector-types-option) option
...
It is possible to implement `vjust` support for `geom_text` in animint, but we haven't
...
Another work-around is to use the new `geom_label_aligned`, which does support `vjust`.
For an example, please read [Chapter 8](../Ch08/Ch08-WorldBank-facets.html).
...
Next, [Chapter 8](../Ch08/Ch08-WorldBank-facets.html) explains how to create a
