...
The code below combines all regions (labels that indicate presence/absence of peaks in specific areas/samples) into a single data table.

```{r}
(all.regions <- rbindlist(PSJ$regions.by.problem))
```

The output above is a table of labeled regions, which display peak detection error rates for different values of `peaks`.

```{r}
show.problems <- data.table(PSJ$problems)[
, y := problem.i/max(problem.i), by=bases.per.problem]
prob.regions$sample.id <- "problems"
all.modelSelection <- rbindlist(PSJ$modelSelection.by.problem)
...
dval2 <- (dval-1)/2 + 1
res.error[, let(
  min.bases.per.problem = bases.per.problem/dval2,
  max.bases.per.problem = bases.per.problem*dval2)]
...
So in this for loop we add a few layers with `aes_string(clickSelects=problem.dot)` or `aes_string(showSelected=problem.dot)` to the `coverage` and `modelSelection` plots.
...
Note the timing of the code above. It takes several seconds just to
...
sample.peaks <- rbindlist(PSJ$peaks.by.problem)[
, selector := peakvar(problem.name)]
problem.peaks <- sample.peaks[, .(
  bases.per.problem, problem.i, problem.name, peaks,
  chromStart, chromEnd, sample.id="problems",
  selector = peakvar(problem.name))]
all.regions[, selector := peakvar(problem.name)]
all.modelSelection[, selector := peakvar(problem.name)]
```

In the code below, the main idea is that for every problem, there is a selector for the number of peaks in that problem.
We use `showSelected=c(selector="peaks")` to indicate that the selector name is found in the `selector` column, and the selection value is found in the `peaks` column. The `animint2dir()` compiler creates a selection variable for every unique value of `selector` (and it uses corresponding values in `peaks` to set/update the selected value/geoms).
...
* Use named clickSelects/showSelected to create a visualization which demonstrates over- and under-fitting, as in [this visualization of linear model and nearest neighbors](https://tdhock.github.io/2023-12-04-degree-neighbors/).
* Use named clickSelects/showSelected to create a visualization of some data from your domain of expertise.

Next, [Chapter 15](/ch15) explains how to visualize
