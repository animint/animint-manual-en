...
* in R Markdown [@R_Markdown] documents.
* using any web server, including Netlify Drop [@Netlify_Drop].
* published using GitHub pages [@GitHub_pages], and
...
code, as explained on our FAQ [@wiki_FAQ].
...
For example I executed the World Bank data viz example code to create the `WorldBank-facets` directory on my personal computer.^[<https://github.com/animint/animint2/blob/master/inst/examples/WorldBank-facets.R>]
I copied that directory to my lab web server using `rsync -r WorldBank-facets/ monsoon.hpc.nau.edu:genomic-ml/WorldBank-facets/` and so I can view it on my university web server.^[<https://rcdata.nau.edu/genomic-ml/WorldBank-facets/>]
...
Netlify Drop [@Netlify_Drop] is for hosting static web
...
Another limitation is 54,000 files per directory, as mentioned in the Deploy Overview docs [@the_Deploy_Overview_docs].

## Publish on GitHub Pages {#pages}

GitHub Pages [@GitHub_Pages] is a service that provides
...
store, easy authentication when pushing to GitHub).
First, install those packages.
If you don't have a GitHub account, you can sign up for free.
...
Before publishing, make sure that your `viz` object has the [`title`](/ch06#title-option) and [`source`](/ch06#source-option) options, which are required by default, in order to encourage interpretability and reproducibility.

To publish your `viz` to the `gh-pages` branch of `your_github_username/new_repo`, you can use `animint2pages(viz, "new_repo")`, where `new_repo` is different from the repo used for the `source` link.
Note that `your_github_username` is not specified in code, because `gitcreds` will get that information from the git credential store.
It should print a message which tells
...
* use `animint2pages(viz, "viz_repo")` to publish that animint to the
  `gh-pages` branch of `your_github_username/viz_repo` (this should be a different repo than the one used to host the source code).
...
## The `animint2` GitHub deployment steps
...
* Begin by writing some visualization code in `figure-name.R` in a GitHub repository, where `name` is an informative name for your data visualization. For example, the tdhock/cs499-spring2020 [@tdhock/cs499-spring2020] GitHub repository contains visualization code in figure-quadratic-interactive.R [@figure-quadratic-interactive.R].
* Commit and push the R code file to GitHub, so you can get a public URL that you can add in your R code as the [`source` option](/ch06#source-option), for example `source="https://github.com/tdhock/cs499-spring2020/blob/master/2020-02-03-capacity/figure-quadratic-interactive.R"`.
* Add a [`title` option](/ch06#title-option) in your R code, for example `title="Overfitting using linear model polynomial degree"`.
* Use [HTML table layout](/ch06#html-table-layout) to make sure that the ggplots in your visualisation always have the same arrangement (in any size web browser window).
* Optionally, create a 2-5 minute screencast video with voice narration that explains how to interpret and interact with the visualization. Upload the video to a site like vimeo [@vimeo] (up to 1GB free hosting without advertisement), then add a [`video` option](/ch06#video-option) in your R code, for example `video="https://vimeo.com/1127992420"`.
...
* If you want to make larger modifications, it is recommended to copy the R code to a new file, for example figure-several-interactive.R [@figure-several-interactive.R] for a new visualization with an additional selector for `pattern`.
...
* The data visualization has links to the corresponding `source` code and explanatory `video`.
* The gallery entry has links to the data visualization, along with its `source` code and explanatory `video`.
* The R code file has a link to the repo that deployed the data visualization, for example:
  * the R code in figure-quadratic-interactive.R [@figure-quadratic-interactive.R]
  * has `2020-02-03-capacity-polynomial-degree` at the bottom on the `animint2pages()` line,
  * and has `tdhock` as the owner of the repo,
  * so it can be inferred that the visualization was deployed to <https://github.com/tdhock/2020-02-03-capacity-polynomial-degree>.
* The data visualization deployment repo has a README.md [@README.md] which has a link to the data visualization, <https://tdhock.github.io/2020-02-03-capacity-polynomial-degree/>.

## Chapter summary and exercises {#ch05-exercises}
...
Next, [Chapter 6](/ch06) explains the different options that
